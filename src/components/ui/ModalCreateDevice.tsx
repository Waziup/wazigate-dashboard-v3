...
export default function CreateDeviceModalWindow(props: CreateDeviceModalWindowProps) {
    ...
    return (
        <Dialog
            fullWidth={fullWidth ? undefined : true}
            open={openModal}
            onClose={handleToggleModal}
            PaperProps={{
                component: 'form', onSubmit: (e: React.FormEvent<HTMLFormElement>) => { e.preventDefault(); submitCreateDevice(e) }
            }}
        >
            <Box sx={{ ...style, width: fullWidth ? 400 : undefined }}>
                ...
                <DialogContent>
                    ...
                    <RowContainerBetween additionStyles={{ my: 1 }}>
                        ...
                        <Android12Switch
                            checked={newDevice.meta.lorawan}
                            onChange={changeMakeLoraWAN}
                            color='secondary'
                        />
                    </RowContainerBetween>
                    {
                        (newDevice.meta.lorawan) && (
                            <Box my={2}>
                                ...
                                <AddTextShow
                                    ...
                                    textInputValue={newDevice.meta.lorawan.devAddr}
                                    onTextInputChange={onTextInputChange}
                                    name="devAddr"
                                    text={'Device Addr (Device Address)'}
                                    placeholder={'8 digits required, got 0'}
                                />
                                <AddTextShow
                                    autoGenerateHandler={autoGenerateLoraWANOptionsHandler}
                                    textInputValue={newDevice.meta.lorawan.nwkSEncKey}
                                    onTextInputChange={onTextInputChange}
                                    name="nwkSEncKey"
                                    text={'NwkSKey(Network Session Key)'}
                                    placeholder={'32 digits required, got 0'}
                                />
                                <AddTextShow autoGenerateHandler={autoGenerateLoraWANOptionsHandler}
                                    textInputValue={newDevice.meta.lorawan.appSKey}
                                    onTextInputChange={onTextInputChange}
                                    name="appSKey"
                                    text={'AppKey (App Key)'}
                                    placeholder={'32 digits required, got 0'}
